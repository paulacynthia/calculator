...
function changeVisibility_class(className, visible) {
  ...
  for (var i = 0; i < N; i++) elements[i].style.visibility = STATE;
}

function htmlchar(c) {
  switch (c) {
    case true:
      return CHARS_SYMBOLS.charTrue;
    case false:
      return CHARS_SYMBOLS.charFalse;
    case '~':
      return '&not;';
    case '&':
      return '&and;';
...
export function construct() {
  console.log('porara meno');
  /* var time = new Date().getTime(); */
  var formulas = document.getElementById('in').value.replace(/ /g, ''); // remove whitespace
  console.log('bola bola', formulas);
  if (formulas == '')
    return alert('Você precisa digitar alguma fórmula bem formada.');

  var r = badchar(formulas);
  if (r >= 0)
    return alert(
      'Você digitou um símbolo não identificado (' + formulas[r] + ')',
    );
  ...
  var text = undefined;
  var latex = undefined;
  var mostrarLinhasCriticas = undefined;
  const mostrarNumeroLinhas = undefined;
  ...
  if (nVariaveis > 12)
    return alert('Você ultrapassou o limite de quantidade de variáveis (12).');

  for (var i = 0; i < nVariaveis; i++) {
    // adds outermost parentheses if needed
    if (trees[i].length == 0) {
  ...
  if (
    trees.filter(function (a) {
      return a.length == 0;
    }).length > 0
  )
    // checks if any formulas are still malformed
  ...
  if (full || main) {
    ...
    if (quantidadeLinhasCriticas == 0)
      textoResultado.innerHTML = 'É consequência lógica!';

    if (validadeArgumento == 'Válido')
      textoResultado.innerHTML = 'É consequência lógica!';
    ...
    document.getElementById('tt').innerHTML =
      '<div class="center"><pre>' + texttable + '</pre></div>';
  } else if (latex) {
    ...
    win.document.close();
    document.getElementById('tt').innerHTML =
      '<div class="center" style="text-align:center;color:red;">LaTex tables open in a new window.<br/>If no window opened, make sure your your browser<br/>isn\'t blocking popups.</div>';
  }
...
function htmlTable(table, trees, flag) {
  ...
  function mkTDrow(tbl, r) {
    ...
    var resp = ''; // 'V' ou 'F'.
    const MARCACAO =
      '<td class="linhaCriticaIdentificador">' + IDENTIFICADOR + '</td>';
      ...
      for (var j = 0; j < tbl[i][r].length; j++) {
        ...
        if (mcs[i - 1] == j) {
          if (TABLE_LENGTH > 2) {
            ...
            if (i == TABLE_LENGTH - 1)
              if ((ehLinhaCritica = verificarLinha(linhaResultante, r)))
                quantidadeLinhasCriticas++;
          }
        ...
        if (j == tbl[i][r].length - 1 && i != TABLE_LENGTH - 1)
  ...
  function verificarLinha(linha, currLine) {
    // linha = array FIFO; verifica se é linha crítica e a validade do argumento.
    ...
    if (linha.shift() == CHARS_SYMBOLS.charFalse) {
      ...
      document.getElementById('validade_argumento').className =
        'argumentoInvalido';
  ...
  function mkrow(tbl, r) {
    ...
    for (var i = 0; i < tbl.length; i++) {
      ...
      for (var j = 0; j < tbl[i][r].length; j++) {
        ...
        if (bcind[i].indexOf(j) >= 0 && r != 0) {
          rw += ' ';
        } // add space for biconditional
...
function mklhs(fs) {
  ...
  if (atomic.indexOf('#') >= 0) {
    ...
    tvrows = tvrows.map(function (x) {
      return [false].concat(x);
    });
  } else {
...
function badchar(s) {
  var x =
    ':,()~v&<>|#ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuwxyz'; /* [v2.0]7 */
  for (var i = 0; i < s.length; i++) {
    if (x.indexOf(s[i]) < 0) {
      return i;
    }
